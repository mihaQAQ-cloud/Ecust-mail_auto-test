"""
草稿箱模块测试（5 用例）
TC-DRAFT-P01 进入草稿箱验证页面加载
TC-DRAFT-P02 查看草稿数量与主题
TC-DRAFT-P03 打开第一封草稿
TC-DRAFT-N01 草稿箱为空时执行删除
TC-DRAFT-N02 未勾选直接点击删除
...
class DraftModuleTest:
    ...
    def _rec(self, cid, ctype, name):
        return {"id": cid, "module": "草稿箱", "type": ctype, "name": name,
    ...
    def _d01(self):
        r = self._rec("TC-DRAFT-P01", "positive", "进入草稿箱验证页面加载")
        self._log("TC-DRAFT-P01 进入草稿箱验证页面加载", "STEP")
        self._nav_to("drafts")
        r["passed"] = True
        r["actual"] = "草稿箱页面加载成功"
        r["screenshot"] = self._shot("d01")
        return r

    def _d02(self):
        r = self._rec("TC-DRAFT-P02", "positive", "查看草稿数量与主题")
        self._log("TC-DRAFT-P02 查看草稿数量与主题", "STEP")
        self._nav_to("drafts")
        cnt = self.nav.count_emails()
        subj = self.nav.get_first_subject() if cnt > 0 else "（无）"
        r["actual"] = f"草稿数={cnt}，主题={subj[:30]}"
        r["passed"] = True
        r["screenshot"] = self._shot("d02")
        return r

    def _d03(self):
        r = self._rec("TC-DRAFT-P03", "positive", "打开第一封草稿")
        self._log("TC-DRAFT-P03 打开第一封草稿", "STEP")
        self._nav_to("drafts")
        if self.nav.count_emails() > 0:
            ...
            r["actual"] = "草稿箱为空（跳过）"
            r["passed"] = True
        r["screenshot"] = self._shot("d03")
        return r

    def _d04(self):
        r = self._rec("TC-DRAFT-N01", "negative", "草稿箱为空时执行删除")
        self._log("TC-DRAFT-N01 草稿箱为空时执行删除", "STEP")
        self._nav_to("drafts")
        if self.nav.is_empty():
            ...
            r["actual"] = f"草稿箱有 {self.nav.count_emails()} 封（非空场景）"
            r["passed"] = True
        r["screenshot"] = self._shot("d04")
        return r

    def _d05(self):
        r = self._rec("TC-DRAFT-N02", "negative", "未勾选直接点击删除")
        self._log("TC-DRAFT-N02 未勾选直接点击删除", "STEP")
        self._nav_to("drafts")
        try:
    ...
    def run(self):
        ...
        print("\n" + "=" * 50)
        print(f"  草稿箱模块测试完成  通过 {passed}/{total}  ({passed/total*100:.0f}%)")
        print("=" * 50)
